...
void generate() {
  ...
  for (int i = 0; i < 200; i++) {
    ...
    xx -= xx%32;
    yy -= yy%32;
    float ss = width*0.1*(0.5+yy/height)*random(2);
    noStroke();
    fill(rcol(), random(50));
    ellipse(xx, yy, ss, ss);

    strokeWeight(random(1, 8)*random(1));
    stroke(rcol());
