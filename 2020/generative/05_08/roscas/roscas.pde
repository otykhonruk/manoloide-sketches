...
void generate() {
  ...
  for (int i = 0; i < 60; i++) {
    ...
    xx -= xx%30;
    yy -= yy%30;
    float ss = width*random(random(0.01, 0.1), 0.6)*0.3*random(1);
    boolean add = true;
    for (int j = 0; j < points.size(); j++) {
      PVector other = points.get(j);
      if (dist(xx, yy, other.x, other.y) < (ss+other.z)*0.51) {
        add = false;
        break;
      }
    }
  ...
  for (int i = 0; i < 18000*5; i++) {
    float xx = width*random(-0.5, 1.5); 
    float yy = height*random(-0.5, 1.5);
    float ss = width*random(random(0.01, 0.1), 0.6)*0.03;
    boolean add = true;
  ...
  for (int i = 0; i < points.size(); i++) {
    ...
    fill(0);
    ellipse(xx, yy, ss*0.06, ss*0.06);
    //fill(rcol());
    //ellipse(xx, yy, ss*0.1-2, ss*0.1-2);
  }
}
