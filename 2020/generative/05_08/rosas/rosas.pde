...
void generate() {
  ...
  for (int i = 0; i < 3400; i++) {
    ...
    if (dist(x, y, width*0.5, height*0.5) > width*0.38) continue;
    ...
    if (add) {
      ...
      for (int k = 0; k < cc; k++) {
        ...
        tint(getColor(col), random(255)*random(0.9, 1)*mult);//, random(260)*random(1)*mult*0.7);//lerpColor(color(0), rcol(), random(0.18)), random(255)*0.1);
