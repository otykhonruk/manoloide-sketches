...
void generate() {
  ...
  int sub = int(random(60, 140))*2;
  float ss = width*2./sub;
  ...
  for (int i = -20; i < sub+20; i++) {
    ...
    for (float j = -height*0.5-100; j < height*0.5+100; j+=0.5) {
      float x1 = (i-sub*0.5)*ss;
      float x2 = x1+ss;
      PVector p1 = def(x1, j, 0);
      PVector p2 = def(x2, j, 0);
  ...
  for (int i = -20; i < sub+20; i++) {
    ...
    //stroke(((i+201)%2)*255);
    beginShape(QUAD_STRIP);
    for (float j = -width*0.5-100; j < width*0.5+100; j+=0.5) {
      float y1 = (i-sub*0.5)*ss;
      float y2 = y1+ss;
      PVector p1 = def(j, y1, 0);
      PVector p2 = def(j, y2, 0);
      vertex(p1.x, p1.y, p1.z);
      vertex(p2.x, p2.y, p2.z);
    }
    endShape(CLOSE);
...
PVector def(float x, float y, float z) {
  ...
  q.x += (noise(desDes+detDes*q.x, desDes+detDes*q.y, 777)*2-1)*120*2;
  q.y += (noise(desDes+detDes*q.x, desDes+detDes*q.y, 333)*2-1)*120*2;

  q.x += (noise(desDes+detDes2*q.x, desDes+detDes2*q.y, 999)*2-1)*30*2;
  q.y += (noise(desDes+detDes2*q.x, desDes+detDes2*q.y, 111)*2-1)*30*2;
