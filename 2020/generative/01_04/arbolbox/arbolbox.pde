...
void generate() {
  ...
  for (int i = 0; i < 90; i++) {
    arbol(random(width), random(height), width*random(0.6)*0.5, height*random(1)*random(0.6, 1)*0.6);
  }
}
...
void arbol(float x, float y, float w, float h) {
  ...
  for (int i = 0; i < rects.size(); i++) {
    ...
    //stroke(120, 250, 20);
    fill(bas, 120);
    lineStr(x1, y1, x2, y2, w*0.001, h*0.001);
  }
  ...
  beginShape(POINTS);
  for (int k = 0; k < w*h*5.8; k++) {
    float xx = x+w*random(-0.5, 0.5);
    float yy = y+h*random(-0.5, 0.5);
    float ss = pow(noise(xx*det, yy*det), 2)*2.8;
    strokeWeight(ss);
    stroke(lerpColor(color(0), getColor(col+random(1)*random(1)+ss*0.6), sqrt(ss*0.1)), random(random(100), 100));
    vertex(xx, yy);
  }
  endShape();
